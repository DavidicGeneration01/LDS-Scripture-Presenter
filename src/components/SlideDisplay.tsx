...
const SlideDisplay: React.FC<SlideDisplayProps> = ({ verse, settings, isFullscreen, toggleFullscreen, isLoading, loadingMessage, isLive }) => {
  const [animateKey, setAnimateKey] = useState(0);
  useEffect(() => { if (verse) setAnimateKey(prev => prev + 1); }, [verse?.reference, verse?.text]);
  ...
  const getAlignmentClass = (align: 'left' | 'center' | 'right') => {
    switch (align) { case 'left': return 'text-left'; case 'right': return 'text-right'; default: return 'text-center'; }
  };

  const containerClass = `relative flex flex-col items-center justify-center w-full h-full p-8 md:p-16 transition-colors duration-700 overflow-hidden ${getThemeClasses(settings.theme)}`;
  ...
  return (
    <div id="presentation-area" className={containerClass}>
      ...
      {isLive && (
        <button onClick={toggleFullscreen} className="absolute top-4 right-4 p-2 rounded-full bg-black/20 hover:bg-black/40 text-white/50 hover:text-white transition-all duration-300 z-50 opacity-0 hover:opacity-100" title={isFullscreen ? 'Exit Fullscreen' : 'Enter Fullscreen'}>
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/60 z-40 backdrop-blur-sm">
          <div className="flex flex-col items-center"><div className="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-blue-400 mb-4"></div><span className="text-blue-200 font-display tracking-widest text-lg animate-pulse">{loadingMessage || 'SEARCHING'}</span></div>
        </div>
      )}

      {verse && (
        <div key={animateKey} className={`z-30 max-w-[90%] w-full ${getAlignmentClass(settings.alignment)} animate-[fadeIn_400ms_ease-out]`}>
          <div className="font-medium mb-8 tracking-wide drop-shadow-lg leading-relaxed text-balance" style={fontSizeStyle}>{verse.text}</div>
          {settings.showReference && (
            <div className="mt-8 border-t border-current/20 pt-6 inline-block w-full"><h2 className="text-5xl md:text-6xl font-display text-blue-200 tracking-wider uppercase">{verse.reference}</h2><p className="text-lg opacity-70 mt-2 uppercase tracking-[0.2em] font-sans font-light">{verse.version}</p></div>
          )}
        </div>
      )}
